    ...
    <div className="mx-auto w-full max-w-3xl space-y-6">
      <header className="flex items-start gap-4 rounded-3xl bg-gradient-to-l from-slate-950 via-slate-900 to-emerald-950 p-6 text-white shadow-xl shadow-slate-900/10">
        ...
        <div>
          ...
          <p className="mt-2 text-sm text-slate-300">اطلاعات شخصی، راه‌های ارتباطی و امنیت حساب خود را مدیریت کنید.</p>
        </div>
      </header>
      ...
      <Card className="overflow-hidden">
        <CardHeader className="border-b border-slate-100 bg-slate-50/70">
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            اطلاعات کاربری و پرسنلی
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          <div className="flex flex-wrap items-center gap-4 border-b border-slate-200 bg-slate-50 p-5">
            {avatarPreview ? (
              <img src={avatarPreview} alt="تصویر پروفایل" className="h-20 w-20 rounded-full object-cover ring-2 ring-white shadow" />
            ) : (
              <div className="grid h-20 w-20 place-items-center rounded-full bg-slate-200 text-slate-500">
                <User className="h-8 w-8" />
              </div>
            )}
            <div className="flex-1">
              <p className="text-sm font-medium text-slate-800">تصویر پروفایل</p>
              <p className="mt-1 text-xs text-slate-500">JPG، PNG یا WebP؛ حداکثر ۵ مگابایت</p>
            </div>
            <input ref={avatarInputRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={handleAvatarUpload} />
            <Button type="button" variant="outline" isLoading={avatarUploading} onClick={() => avatarInputRef.current?.click()}>
              بارگذاری تصویر
            </Button>
          </div>
          ...
          {profilesLoading ? (
            <div className="border-t border-slate-200 py-6 text-center text-sm text-slate-500">در حال دریافت اطلاعات پرسنلی...</div>
          ) : !employeeProfiles ? (
            <p className="border-t border-slate-200 px-6 py-5 text-sm text-slate-500">اطلاعات پرسنلی در دسترس نیست.</p>
          ) : (
            <EmployeeProfileSettings embedded tenant={{ id: 'global', name: 'همه سازمان‌های شما' }} employee={employeeProfiles.profile} />
          )}
        </CardContent>
      </Card>
      ...
      <Card className="overflow-hidden">
        <CardHeader className="border-b border-slate-100 bg-slate-50/70">
          ...
          <p className="mt-1 text-xs text-slate-500">تصویر کارت ملی به حساب کاربری شما تعلق دارد و مستقل از سازمان‌ها نگهداری می‌شود.</p>
        </CardHeader>
        <CardContent className="space-y-4">
          {documentsLoading ? (
            <div className="py-4 text-center text-sm text-slate-500">در حال دریافت اسناد...</div>
          ) : userDocuments.length === 0 ? (
            <p className="text-sm text-slate-500">هنوز سندی برای حساب شما ثبت نشده است.</p>
          ) : (
            userDocuments.map((document) => (
              <div key={document.id} className="flex flex-wrap items-center gap-3 rounded-2xl border border-slate-200 p-4">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-slate-800">{document.kind === 'national_card' ? 'تصویر کارت ملی' : document.originalName}</p>
                  <p className="mt-1 text-xs text-slate-500">{document.originalName} · {formatFileSize(document.size)}</p>
                </div>
                <a className="inline-flex items-center gap-1 rounded-xl border border-slate-300 px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-50" href={withBasePath(`/api/auth/user-documents/${document.id}/file`)} target="_blank" rel="noreferrer">
                  <ExternalLink className="h-4 w-4" />
                  مشاهده
                </a>
                <Button type="button" variant="danger" size="sm" onClick={() => void handleUserDocumentDelete(document.id)}>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Button type="button" variant="outline" isLoading={documentUploading} onClick={() => nationalCardInputRef.current?.click()}>

  ...
  return (
    <div className="mx-auto max-w-7xl space-y-5">
      <section className="rounded-3xl bg-slate-950 p-6 text-white">
        <h2 className="text-2xl font-bold">مدیریت پلتفرم</h2>
        <p className="mt-2 text-sm text-slate-300">کاربران مستقل، عضویت‌های سازمانی، سازمان‌ها و مالکیت‌ها</p>
      </section>
      {overview && (
        ...
        <CardContent>
          ...
          {isSuperAdmin && showCreateUser && (
            <form onSubmit={createUser} className="mb-5 grid gap-3 rounded-2xl border border-primary-100 bg-primary-50/40 p-4 sm:grid-cols-2">
              <Input label="نام و نام خانوادگی" value={newUser.name} onChange={(event) => setNewUser((current) => ({ ...current, name: event.target.value }))} required />

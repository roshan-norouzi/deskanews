...
const STAGE_LABELS: Record<string, string> = {
  ...
  routed: 'ارسال‌شده به استودیو',
  published: 'منتشرشده',
  failed: 'نیازمند رسیدگی',
  archived: 'آرشیو',
  rejected: 'ردشده',
};
...
function DashboardContent() {
  ...
  const stats = [
    {
      title: 'ورودی اتاق خبر',
      value: data.publishing.newsroom.inbox,
      icon: Rss,
      tone: 'bg-blue-50 text-blue-700',
    },
    {
      title: 'ورودی استودیوی اجتماعی',
      value: data.publishing.social.inbox,
      icon: Send,
      tone: 'bg-violet-50 text-violet-700',
    },
    {
      title: 'کارهای صف',
      value: data.publishing.queue.queued + data.publishing.queue.running,
      icon: Bot,
      tone: 'bg-cyan-50 text-cyan-700',
    },
    {
      title: 'فرایندهای متوقف‌شده',
      value: data.publishing.queue.dead,
      icon: Activity,
      tone: data.publishing.queue.dead ? 'bg-red-50 text-red-700' : 'bg-emerald-50 text-emerald-700',
    },
    {
      title: 'اتصال‌های ناسالم',
      value: data.publishing.unhealthyIntegrations,
      icon: AlertTriangle,
      tone: data.publishing.unhealthyIntegrations
    ...
    },
    {
      ...
      icon: Bell,
      tone: data.notifications.unread ? 'bg-amber-50 text-amber-700' : 'bg-slate-50 text-slate-700',
    },
  ];
    ...
    <div className="mx-auto w-full max-w-7xl space-y-6" dir="rtl">
      <header className="flex flex-col gap-4 rounded-3xl bg-gradient-to-l from-slate-950 via-slate-900 to-blue-950 p-6 text-white shadow-xl shadow-slate-900/10 sm:flex-row sm:items-start">
        ...
        <div className="flex-1">
          <h2 className="text-2xl font-bold">میز کار نشر</h2>
          <p className="mt-2 text-sm text-slate-300">
            وضعیت اتاق خبر، استودیوی اجتماعی، صف اتوماسیون و موارد نیازمند اقدام
          </p>
        ...
        <Link
          ...
          className="rounded-xl bg-white/10 px-4 py-2 text-sm font-medium ring-1 ring-white/20 hover:bg-white/15"
        >
          مرکز عملیات
        </Link>
      </header>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <PublishingSummary
          ...
          icon={<Rss className="h-5 w-5 text-blue-700" />}
          values={[
        ...
        <PublishingSummary
          ...
          icon={<Send className="h-5 w-5 text-violet-700" />}
          values={[
      ...
      <section className="grid gap-6 xl:grid-cols-[1.5fr_1fr]">
        <Card className="overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-slate-900">صندوق کار</h3>
            </div>
            <span className="text-xs text-slate-500">
              {formatPersianDigits(data.workItems.length)} مورد اخیر
            </span>
          </div>
          {data.workItems.length ? (
                  ...
                  <div className="flex flex-wrap items-start gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        {item.subtitle} · {formatJalaliDateTime(item.updatedAt)}
                      </p>
                      {item.error && (
                    ...
                    <Badge variant={stageVariant(item.stage)}>
                      {STAGE_LABELS[item.stage] || item.status}
                    </Badge>
                  </div>
                </Link>
              ))}
            ...
            <div className="grid place-items-center px-5 py-14 text-center">
              ...
              <p className="mt-3 font-medium text-slate-800">مورد معطل یا خطاداری وجود ندارد</p>
            </div>
          )}
        </Card>
        ...
        <Card className="overflow-hidden">
          <div className="border-b border-slate-100 px-5 py-4">
            <h3 className="font-bold text-slate-900">آخرین فعالیت‌ها</h3>
          </div>
  ...
  return (
    <Card className="p-5">
      <div className="flex items-center justify-between">
        ...
        <Link href={href} className="text-sm text-primary-700">مشاهده</Link>
      </div>
      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-5">
        {values.map(([label, value]) => (
          <div key={label} className="rounded-xl bg-slate-50 p-3 text-center">
            <p className="text-xs text-slate-500">{label}</p>
            <p className="mt-1 text-xl font-bold">{formatPersianDigits(value)}</p>
          </div>
        ))}
      </div>
    </Card>

...
export default function SocialPage() {
  ...
  const articlesApi = useApi<{ items: Article[]; nextCursor: string | null }>(showArchived ? `/publishing/social/articles?status=archived${socialCursor ? `&cursor=${encodeURIComponent(socialCursor)}` : ''}` : `/publishing/social/articles${socialCursor ? `?cursor=${encodeURIComponent(socialCursor)}` : ''}`);
  ...
  async function publishToNetwork(network: Network) {
    ...
    try {
      ...
      const result = await apiFetch<{ message?: string; queued?: boolean }>(`/publishing/social/articles/${publishArticle.id}/publish/${network}`, { method: 'POST', body: { caption, imageDataUrl: image.dataUrl } });
      const queuedText = QUEUED_JOB_SUCCESS;
      setNetworkResults((current) => ({ ...current, [network]: result.queued ? queuedText : (result.message || 'با موفقیت منتشر شد.') }));
      setNotice({ type: 'success', text: result.queued ? queuedText : (image.usedFeaturedFallback ? `تولید یا دریافت تصویر قالبی انجام نشد؛ مطلب با تصویر شاخص در ${NETWORKS.find((item) => item.id === network)?.label} منتشر شد.` : `مطلب در ${NETWORKS.find((item) => item.id === network)?.label} منتشر شد.`) });
    }
  ...
  return <ProtectedLayout><PageContainer>
    <PageHeader
      title="استودیوی اجتماعی"
      description="مطالب میز خبر یا منابع اختصاصی استودیو را با قالب کپشن و تصویر میز خبر آماده کنید و در شبکه‌های اجتماعی منتشر کنید؛ با اقدام شما یا به‌صورت خودکار."
      icon={Share2}
      actions={
        <>
          ...
          <Button variant="danger" isLoading={busy === 'delete-all'} onClick={async () => { const ok = await confirm({ title: 'حذف همه مطالب استودیو؟', description: 'همه مطالب استودیوی اجتماعی برای همیشه حذف می‌شوند.', confirmLabel: 'حذف همه مطالب', variant: 'danger' }); if (!ok) return; void run('delete-all', () => apiFetch('/publishing/social/articles', { method: 'DELETE' }), 'همه مطالب استودیوی اجتماعی حذف شدند.'); }}><Trash2 className="h-4 w-4" /> حذف همه مطالب</Button>

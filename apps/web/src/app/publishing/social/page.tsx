...
export default function SocialPage() {
  ...
  return <ProtectedLayout title="استودیوی اجتماعی"><main className="mx-auto w-full max-w-7xl space-y-6 p-4 sm:p-6" dir="rtl">
    <header className="flex flex-col gap-5 rounded-3xl bg-gradient-to-l from-slate-950 via-slate-900 to-violet-950 p-6 text-white shadow-xl shadow-slate-900/10 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-start gap-4"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-white/10 ring-1 ring-white/15"><Share2 className="h-6 w-6" /></span><div><h1 className="text-2xl font-bold">استودیوی اجتماعی</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">مدیریت محتوای شبکه‌های اجتماعی.</p></div></div><div className="flex flex-wrap gap-2"><Link href="/publishing/feeds" className="inline-flex items-center gap-2 rounded-xl border border-white/20 bg-white/10 px-4 py-2 text-sm hover:bg-white/15"><Rss className="h-4 w-4" /> مدیریت منابع</Link><Link href="/publishing/settings" className="inline-flex items-center gap-2 rounded-xl border border-white/20 bg-white/10 px-4 py-2 text-sm hover:bg-white/15"><Settings2 className="h-4 w-4" /> تنظیمات انتشار</Link><Button variant="outline" className="border-white/30 bg-white/10 text-white hover:bg-white/20 hover:text-white" onClick={toggleArchivedView}><Archive className="h-4 w-4" /> {showArchived ? 'بازگشت به مطالب فعال' : 'نمایش مطالب آرشیوشده'}</Button><Button variant="ghost" className="border border-red-300/40 text-red-100 hover:bg-red-500/20" isLoading={busy === 'delete-all'} onClick={() => { if (window.confirm('همه مطالب استودیوی اجتماعی برای همیشه حذف شوند؟')) void run('delete-all', () => apiFetch('/publishing/social/articles', { method: 'DELETE' }), 'همه مطالب استودیوی اجتماعی حذف شدند.'); }}><Trash2 className="h-4 w-4" /> حذف همه مطالب</Button><Button className="bg-white text-slate-900 hover:bg-slate-100" isLoading={busy === 'sync'} onClick={() => run('sync', () => apiFetch('/publishing/social/sync', { method: 'POST' }), 'منابع پایش شدند؛ مطالب جدید در صف آماده‌سازی قرار گرفتند.')}><RefreshCw className="h-4 w-4" /> پایش منابع</Button></div></header>
    ...
    {feeds.length === 0 && <Card className="flex min-h-48 flex-col items-center justify-center p-8 text-center"><Rss className="h-9 w-9 text-slate-300" /><h2 className="mt-4 font-bold text-slate-900">فید اجتماعی ثبت نشده است</h2><p className="mt-2 text-sm text-slate-500">از صفحه فیدها یک منبع با کاربرد «استودیوی اجتماعی» اضافه کنید.</p><Link href="/publishing/feeds" className="mt-5 rounded-xl bg-violet-600 px-4 py-2 text-sm font-semibold text-white">افزودن فید</Link></Card>}

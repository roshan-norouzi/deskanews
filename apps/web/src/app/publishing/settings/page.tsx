    ...
    <ProtectedLayout ownerOnly>
      <PageContainer>
        <PageHeader
          ...
          description="پایش خودکار، اتصال سایت و حساب‌های اجتماعی و قالب‌های تصویر را برای این میز خبر تنظیم کنید."
          icon={Settings2}
        />
        ...
        {activeTab === 'social' && <Card className="p-5 sm:p-6">
          <div className="flex items-start gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-violet-50 text-violet-700"><Share2 className="h-5 w-5" /></span><div><h2 className="text-lg font-bold text-slate-900">قالب‌های استودیوی اجتماعی</h2><p className="mt-1 text-sm text-slate-500">قالب کپشن و قالب تصویری میز خبر را تنظیم کنید تا مطالب استودیو با ساختار و ظاهر دلخواه شما آماده شوند. آماده‌سازی و انتشار خودکار را هم از همین بخش تنظیم کنید.</p></div></div>
          {subTab === 'monitor' && <div className="mt-6 space-y-5">
            ...
            <div className="rounded-2xl border border-violet-100 bg-violet-50/40 p-4"><h3 className="font-bold text-slate-900">خودکارسازی استودیوی اجتماعی</h3><p className="mt-1 text-xs leading-5 text-slate-600">برای خودکارشدن مسیر دریافت تا انتشار، مراحل موردنیاز را فعال و حساب‌های مقصد را متصل کنید. هر مرحله مستقل است. با فعال‌کردن تولید تصویر، کاور از قالب انتخابی ساخته می‌شود؛ در غیر این صورت تصویر شاخص منبع استفاده خواهد شد.</p><div className="mt-4 grid gap-3 md:grid-cols-2">
              ...
              <AutomationToggle title="آماده‌سازی خودکار مطالب" description="لید، خلاصه و کپشن مطالب جدید به‌صورت خودکار آماده شوند." enabled={values.social_auto_prepare === 'true'} onChange={(enabled) => set('social_auto_prepare', String(enabled))} />
              <AutomationToggle title="تولید خودکار تصویر از قالب" description="پس از آماده‌شدن مطلب، تصویر نهایی با قالب انتخاب‌شده ساخته و برای انتشار خودکار استفاده شود." enabled={values.social_auto_generate_image === 'true'} onChange={(enabled) => set('social_auto_generate_image', String(enabled))} />
        ...
        {activeTab === 'news' && <Card className="p-5 sm:p-6">
          ...
          {subTab === 'schedule' && <div className="mt-6 space-y-5">
            <div className="rounded-2xl border border-amber-100 bg-amber-50/70 px-4 py-3 text-sm leading-6 text-amber-900">
              با ذخیره این بخش، منابع میز خبر و اشتراک‌های کاتالوگ که «پیش‌فرض میز خبر» دارند بلافاصله از مقادیر جدید پیروی می‌کنند. برای تفاوت روی یک منبع، در «منابع خبری» حالت اختصاصی را فعال کنید.
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              <Field label="فاصله پایش (دقیقه)" hint="پیش‌فرض ۲۴۰ دقیقه است. مقدار معتبر بین ۵ تا ۱۴۴۰ دقیقه است."><input type="number" min="5" max="1440" inputMode="numeric" dir="ltr" className="rounded-xl border px-3 py-2.5" value={values.news_poll_interval_minutes || '240'} onChange={(e) => set('news_poll_interval_minutes', e.target.value)} /></Field>
              <Field label="حداکثر قدمت خبر (روز)" hint="پیش‌فرض ۲ روز است. کاتالوگ سراسری ۳ روز به زبان اصلی نگه می‌دارد؛ اگر این مقدار بیشتر باشد، بقیه از خود منبع گرفته می‌شود."><input type="number" min="1" max="90" inputMode="numeric" dir="ltr" className="rounded-xl border px-3 py-2.5" value={values.news_max_age_days || '2'} onChange={(e) => set('news_max_age_days', e.target.value)} /></Field>
            </div>
            <div className="rounded-2xl border border-blue-100 bg-blue-50/50 p-4"><h3 className="font-bold text-slate-900">خودکارسازی میز خبر</h3><p className="mt-1 text-xs leading-5 text-slate-600">زبان هر خبر پیش از آماده‌سازی تشخیص داده می‌شود؛ خبر فارسی بازنویسی و خبر خارجی ترجمه خواهد شد.</p><div className="mt-4 grid gap-3 md:grid-cols-2">
              ...
              <AutomationToggle title="آماده‌سازی خودکار خبرها" description="تیتر و خلاصهٔ خبرهای جدید، متناسب با زبان منبع، ترجمه یا بازنویسی شوند." enabled={values.news_auto_prepare !== 'false'} onChange={(enabled) => set('news_auto_prepare', String(enabled))} />
              ...
              <AutomationToggle title="ارسال خودکار به استودیوی اجتماعی" description="خبرهای آماده به استودیوی اجتماعی منتقل شوند تا محتوای شبکه‌های اجتماعی برای آن‌ها آماده شود. در این حالت، خبر به سایت ارسال نمی‌شود." enabled={values.news_auto_send_social === 'true'} onChange={(enabled) => { set('news_auto_send_social', String(enabled)); if (enabled) set('news_auto_publish', 'false'); }} />
            </div></div>
          </div>}
          {subTab === 'schedule' && (
        ...
        {activeTab === 'news' && subTab === 'destination' && <Card className="p-5 sm:p-6">
          ...
          <section className="mt-6 rounded-2xl border border-sky-100 bg-sky-50/60 p-4 sm:p-5">
            ...
            {destinationCategoryRows.length ? (
              ...
              <p className="mt-3 text-xs leading-5 text-slate-600">آدرس سایت را وارد کنید و «همگام‌سازی دسته‌بندی‌ها» را انتخاب کنید.</p>
            )}
          </section>
          ...
          <section className="mt-6 space-y-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">برای ارسال خبر به سایت، اطلاعات اتصال و دسترسی انتشار را تکمیل کنید. فعال‌کردن انتشار خودکار از بخش «پایش خبر» انجام می‌شود.</p>
            </div>
            <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <Button variant="outline" isLoading={busy === 'destination'} onClick={() => void testDestinationConnection()}>
              <TestTube2 className="h-4 w-4" /> بررسی اتصال انتشار {destinationMeta.label}
            </Button>

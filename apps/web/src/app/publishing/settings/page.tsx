...
export default function PublishingSettingsPage() {
  ...
  async function testWordPressConnection() {
    setBusy('wordpress'); setError(''); setMessage('');
    try {
      const response = await apiFetch<{ ok: true; message?: string; categories?: WordPressCategory[] }>('/publishing/settings/test-wordpress', {
        method: 'POST',
        body: editableSettings(Object.fromEntries(WORDPRESS_CONNECTION_KEYS.map((key) => [key, values[key] ?? '']))),
      });
      ...
      setError(reason instanceof ApiError ? reason.message : 'آزمایش اتصال WordPress انجام نشد');
    } finally { setBusy(null); }
  }
      ...
      <main className="mx-auto w-full max-w-6xl space-y-6 p-4 sm:p-6" dir="rtl">
        ...
        {activeTab === 'news' && <Card className="p-5 sm:p-6">
          ...
          {subTab === 'schedule' && <div className="mt-6 space-y-5">
            ...
            <div className="rounded-2xl border border-blue-100 bg-blue-50/50 p-4"><h3 className="font-bold text-slate-900">اتوماسیون اتاق خبر</h3><p className="mt-1 text-xs leading-5 text-slate-600">زبان هر خبر پیش از آماده‌سازی تشخیص داده می‌شود؛ خبر فارسی بازنویسی و خبر خارجی ترجمه خواهد شد.</p><div className="mt-4 grid gap-3 md:grid-cols-2">
              ...
              <AutomationToggle title="انتشار خودکار در سایت" description="خبر آماده مستقیماً با متن کامل پردازش‌شده به WordPress ارسال شود؛ اتصال WordPress باید کامل باشد." enabled={values.news_auto_publish === 'true'} onChange={(enabled) => { set('news_auto_publish', String(enabled)); if (enabled) set('news_auto_send_social', 'false'); }} />
              <AutomationToggle title="ارسال خودکار به استودیوی اجتماعی" description="خبر آماده مستقیماً برای شبکه‌های اجتماعی آماده شود؛ در این حالت به WordPress ارسال نمی‌شود." enabled={values.news_auto_send_social === 'true'} onChange={(enabled) => { set('news_auto_send_social', String(enabled)); if (enabled) set('news_auto_publish', 'false'); }} />
            </div></div>
          </div>}
          {subTab === 'prompts' && <div className="mt-5 grid gap-5">
            <Field label="پرامپت تیتر و خلاصهٔ خبر خارجی" hint="برای ترجمه و خلاصه‌سازی خبرهای غیرفارسی استفاده می‌شود."><textarea className="min-h-36 rounded-xl border px-3 py-3 leading-7" placeholder="لحن، دقت، واژگان و قواعد ترجمه و خلاصه‌سازی خبر خارجی را مشخص کنید..." value={values.news_summary_prompt || ''} onChange={(e) => set('news_summary_prompt', e.target.value)} /></Field>
            <Field label="پرامپت متن کامل خبر خارجی" hint="هنگام انتشار در سایت، متن کامل خبر غیرفارسی با این دستور ترجمه می‌شود."><textarea className="min-h-36 rounded-xl border px-3 py-3 leading-7" placeholder="قواعد ترجمهٔ کامل، دقیق و بدون حذف جزئیات را مشخص کنید..." value={values.news_full_translation_prompt || ''} onChange={(e) => set('news_full_translation_prompt', e.target.value)} /></Field>
            <Field label="پرامپت تیتر و خلاصهٔ خبر فارسی" hint="برای بازنویسی حرفه‌ای تیتر و خلاصه خبرهایی که فارسی تشخیص داده می‌شوند."><textarea className="min-h-36 rounded-xl border px-3 py-3 leading-7" placeholder="لحن، ساختار تیتر، طول خلاصه و قواعد بازنویسی خبر فارسی را مشخص کنید..." value={values.news_persian_rewrite_prompt || ''} onChange={(e) => set('news_persian_rewrite_prompt', e.target.value)} /></Field>
            <Field label="پرامپت متن کامل خبر فارسی" hint="هنگام انتشار در سایت، متن کامل خبر فارسی با این دستور بازنویسی می‌شود."><textarea className="min-h-36 rounded-xl border px-3 py-3 leading-7" placeholder="قواعد بازنویسی کامل متن فارسی با حفظ نام‌ها، اعداد و نقل‌قول‌ها را مشخص کنید..." value={values.news_persian_full_rewrite_prompt || ''} onChange={(e) => set('news_persian_full_rewrite_prompt', e.target.value)} /></Field>
          </div>}
        ...
        {activeTab === 'news' && subTab === 'wordpress' && <Card className="p-5 sm:p-6">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <Field label="مسیر صفحه ورود WordPress" hint="اگر صفحه ورود سفارشی است، فقط مسیر آن را وارد کنید. این گزینه در اتصال REST API نقشی ندارد."><div className="flex items-center gap-2"><span dir="ltr" className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-500">/</span><input dir="ltr" className="min-w-0 flex-1 rounded-xl border px-3 py-2.5" placeholder="wp-admin" value={values.wp_login_path || 'wp-admin'} onChange={(e) => set('wp_login_path', e.target.value.replace(/^\/+/, ''))} /></div></Field>

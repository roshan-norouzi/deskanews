  ...
  async prepare(tenantId: string, id: string) {
    ...
    try {
      ...
      return await this.prisma.socialArticle.updateMany({
        ...
        if (updated) await this.recordWorkflow({ tenantId, id, fromStatus: 'processing', toStatus: updated.status, action: 'prepared', title: `مطلب اجتماعی «${updated.title}» آماده شد` });
        return updated;
      });
    } catch (error) {

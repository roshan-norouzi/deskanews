...
test('newsroom publishes with the category selected from live WordPress categories', async () => {
  ...
  const newsroom = new NewsroomService(prisma, settings, gapGpt, sourceReader, {}, wordpress, {}, integrationHealth, workflow, platformFeeds, usageTracking);
...
test('news automation durably queues social routing instead of publishing to WordPress', async () => {
  ...
  const newsroom = new NewsroomService(prisma, settings, {}, {}, {}, {}, jobs, integrationHealth, workflow, platformFeeds, usageTracking);
...
test('server cover renderer stores the selected visual template result on the article', async () => {
  ...
  const renderer = new SocialCoverRendererService(prisma, {}, {}, publisher);

...
test('saving a new integration host removes an omitted stored secret', async () => {
  ...
  const prisma = {
    tenant: {
      findUnique: async () => ({ settings: {
        publishing: {
          wp_site_url: 'https://trusted-wp.example',
          wp_app_password: 'encrypted-stored-secret',
        },
      } }),
      update: async ({ data }) => {
  ...
  service.getRaw = async () => ({
    wp_site_url: 'https://trusted-wp.example',
    wp_app_password: 'stored-secret',
  });
  service.getPublic = async () => ({});

  await service.save('tenant-a', { wp_site_url: 'https://new-wp.example' });

  assert.equal(writtenSettings.wp_site_url, 'https://new-wp.example');
  assert.equal(Object.hasOwn(writtenSettings, 'wp_app_password'), false);
});
